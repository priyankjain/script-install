	...
	if($_REQUEST['addAnswer'] == 'Add Another Answer') {
		if($_REQUEST['questionType'] == 'radio' || ($_REQUEST['questionType'] == '' && $_SESSION['answerType'] == 'radio')) {
			if($_REQUEST['radioAnswer'] != '') {
				...
				echo "<script type=\"text/javascript\">alert('You cannot enter a blank radio button answer');</script>";
			}
			...
			if($_REQUEST['dropdownAnswer'] != '') {
				...
				echo "<script type=\"text/javascript\">alert('You cannot enter a blank drop down menu answer');</script>";
			}
			...
			if($_REQUEST['checkboxAnswer'] != '') {
				...
				echo "<script type=\"text/javascript\">alert('You cannot enter a blank check box answer');</script>";
			}
		...
		if($_SESSION['answerType'] == 'radio' || $_REQUEST['answerType'] == 'dropdown' || $_REQUEST['answerType'] == 'checkbox') {
			if(count($_SESSION['answers']) > 1) {
				...
				echo "<script type=\"text/javascript\">alert('You must have at least 2 answers for your question answer type.');</script>";
			...
			if(is_array($_SESSION['surveyquestions'])) {
				foreach($_SESSION['surveyquestions'] as $k => $v) {
					if($_REQUEST['question'] == $v['question']) {
						$continueScript = FALSE;
						echo "<script type=\"text/javascript\">alert('Error: Duplication Question\\n\\nThis question is already in this survey.');</script>";
						break;
					}
				}
			}
		...
		*/
		...
		if(isset($_SESSION['surveyquestions'])) {
			...
			echo "Could not add your survey!";
		}
...
if($act=='addfad') {
	...
	if(getValue("SELECT COUNT(fnum) FROM featuredads WHERE fname = ".quote_smart($_REQUEST['name'])." AND description = ".quote_smart($description)." AND flink = ".quote_smart($flink)." AND ftitle = ".quote_smart($ftitle)." AND username = ".quote_smart($_SESSION['login'])."") == 0) {
		mysql_query("INSERT INTO featuredads (fnum, fname, femail, fsize, description, flink, ftitle, flogin, fpassword,username) VALUES ('', ".quote_smart($_REQUEST['name']).", ".quote_smart($email).", ".quote_smart(0).", ".quote_smart($description).", ".quote_smart($flink).", ".quote_smart($ftitle).", ".quote_smart($_SESSION['login']).", ".quote_smart($adPassword).", ".quote_smart($_SESSION['login'])." )") or die(mysql_error());
	}
	echo"<script> alert('NEW BANNER HAS BEEN ADDED'); </script>";
}
...
if($act=='addflink') {
	$description = substr($description, 0, 40);
	$_REQUEST['name'] = substr($_REQUEST['name'], 0, 15);
	if(getValue("SELECT COUNT(fnum) FROM featuredlinks WHERE fname = ".quote_smart($_REQUEST['name'])." AND flink = ".quote_smart($flink)." AND ftitle = ".quote_smart($ftitle)." AND username = ".quote_smart($_SESSION['login'])."") == 0) {
		mysql_query("INSERT INTO featuredlinks (fnum, fname, femail, fsize, flink, ftitle, flogin, fpassword,username) VALUES ('', ".quote_smart($_REQUEST['name']).", ".quote_smart($email).", ".quote_smart(0).", ".quote_smart($flink).", ".quote_smart($ftitle).", ".quote_smart($_SESSION['login']).", ".quote_smart($adPassword).", ".quote_smart($_SESSION['login'])." )") or die(mysql_error());
	}
	echo"<script> alert('NEW BANNER HAS BEEN ADDED'); </script>";
}
...
//BANNER SQL CALLS
if($act == 'addBanner'){
	if(getValue("SELECT COUNT(fnum) FROM banners WHERE fname = ".quote_smart($_REQUEST['name'])." AND femail = ".quote_smart($_REQUEST['email'])." AND flink = ".quote_smart($flink)." AND furl = ".quote_smart($_REQUEST['furl'])." AND username = ".quote_smart($_SESSION['login'])."") == 0) { 
		mysql_query("INSERT INTO banners (fname, femail, fsize, fbanercode, flink, furl, flogin, fpassword, username)
		VALUES
		(".quote_smart($_REQUEST['name']).", ".quote_smart($email).", ".quote_smart('0').", ".quote_smart("<a href=\"$flink\"><img src=\"".$_REQUEST['furl']."\" width=\"482\" height=\"60\"></a>").", ".quote_smart($flink).", ".quote_smart($_REQUEST['furl']).", ".quote_smart($_SESSION['login']).", ".quote_smart($adPassword).",".quote_smart($_SESSION['login']).")") or die(mysql_error());
		echo"<script> alert('NEW BANNER HAS BEEN ADDED'); </script>";
	}
}
...
if($act=='addfbanner'){
	if(getValue("SELECT COUNT(fnum) FROM fbanners WHERE fname = ".quote_smart($_REQUEST['name'])." AND femail = ".quote_smart($_REQUEST['email'])." AND flink = ".quote_smart($flink)." AND furl = ".quote_smart($_REQUEST['furl'])." AND username = ".quote_smart($_SESSION['login'])."") == 0) { 
		mysql_query("INSERT INTO fbanners (fname, femail, fsize, fbanercode, flink, furl, flogin, fpassword,username) VALUES (".quote_smart($_REQUEST['name']).", ".quote_smart($email).", ".quote_smart(0).", ".quote_smart($bancode).", ".quote_smart($flink).", ".quote_smart($_REQUEST['furl']).", ".quote_smart($_SESSION['login']).", ".quote_smart($adPassword).",".quote_smart($_SESSION['login'])." )") or die(mysql_error());
		echo"<script> alert('NEW BANNER HAS BEEN ADDED'); </script>";
	} else {
...
//LINKS SQL CALLS
if($act=='addlinks'){
	$qr = "SELECT COUNT(fn) FROM tasks WHERE fsitename = ".quote_smart($_REQUEST['sitename'])." AND furl = ".quote_smart($_REQUEST['url'])." AND username = ".quote_smart($_SESSION['login'])."";
	//echo "Running " . $qr."<HR>";
	$cnt = getValue($qr);
	//echo "Cnt=".$cnt."<BR>";
	if($cnt == 0) { 
		...
		VALUES('0',".quote_smart($_REQUEST['url']).",".quote_smart($_REQUEST['sitename']).",0,".quote_smart($_SESSION['login']).",".quote_smart($adPassword).",'','','1','points',".quote_smart($_SESSION['login']).")";
		//echo "Running Query: ".$query;
		$sql = mysql_query($query) or die(mysql_error());
		if(mysql_affected_rows()) echo"<script> alert('NEW VISIT CAMPAIGN ADDED SUCCESSFULLY'); </script>";
	}
}
//PTR AD SQL CALLS
if($act=='addptrad'){
	$qr = "SELECT COUNT(fn) FROM ptrads WHERE fsitename = ".quote_smart($_REQUEST['sitename'])." AND furl = ".quote_smart($_REQUEST['url'])." AND username = ".quote_smart($_SESSION['login'])."";
	//echo "Running " . $qr."<HR>";
	$cnt = getValue($qr);
	//echo "Cnt=".$cnt."<BR>";
	if($cnt == 0) { 
		$query = "INSERT INTO ptrads
		(fsize,furl,fsitename,fvisits,flog,fpass,startdate,enddate,prise,fpaytype,username,ptrad)
		VALUES('0',".quote_smart($_REQUEST['url']).",".quote_smart($_REQUEST['sitename']).",0,".quote_smart($_SESSION['login']).",".quote_smart($adPassword).",'','','1','points',".quote_smart($_SESSION['login']).",".quote_smart($_REQUEST['ptrad']).")";
		//echo "Running Query: ".$query;
		$sql = mysql_query($query) or die(mysql_error());
		if(mysql_affected_rows()) echo"<script> alert('NEW VISIT CAMPAIGN ADDED SUCCESSFULLY'); </script>";
	} else {
		echo"<script> alert('Error: An ad with this name already exists.'); </script>";
	}
}

if($act=='addsignup'){
	if(getValue("SELECT COUNT(fnum) FROM signups WHERE fsitename = ".quote_smart($_REQUEST['sitename'])." AND fname = ".quote_smart($_REQUEST['fname'])." AND furl = ".quote_smart($url)." AND username = ".quote_smart($_SESSION['login'])."") == 0) { 
		$sql=mysql_query("
		INSERT INTO signups
		(fmail, fname, fsitename, fnote, fsize, furl, flogin, fpassword, fprise, fpaytype, squest, sansw,username)
		VALUES
		(".quote_smart($email).", ".quote_smart($_REQUEST['name']).", ".quote_smart($_REQUEST['sitename']).", ".quote_smart($note).", ".quote_smart('0').", ".quote_smart($url).", ".quote_smart($_SESSION['login']).", ".quote_smart($adPassword).", '1', 'points', ".quote_smart($squest).", ".quote_smart($sansw).",".quote_smart($_SESSION['login']).")")or die(mysql_error());
		if(mysql_affected_rows())echo"<script> alert('NEW SIGNUP CAMPAIGN ADDED SUCCESFULLY!'); </script>";
	}
}
if($act=='addemail'){
	if(getValue("SELECT COUNT(`fnum`) FROM `reads` WHERE `furl` = ".quote_smart($_REQUEST['mailurl'])." AND `fsubject` = ".quote_smart($_REQUEST['subject'])." AND `ftext` = ".quote_smart($_REQUEST['mailcontent'])." AND `username` = ".quote_smart($_SESSION['login'])."") == 0) { 
 		mysql_query("INSERT INTO `reads` (`furl`, `fsubject`, `ftext`, `fprise`, `flog`, `fpass`, `fsize`, `fpaytype`,`username`, `mailcoding`, `mailformat`) VALUES (".quote_smart($_REQUEST['mailurl']).", ".quote_smart($_REQUEST['subject']).", ".quote_smart($_REQUEST['mailcontent']).", ".quote_smart($_REQUEST['prise']).", ".quote_smart($_REQUEST['log']).", ".quote_smart($_REQUEST['pass']).", '0', ".quote_smart($_REQUEST['paytype']).",".quote_smart($_SESSION['login']).",".quote_smart($_REQUEST['mailcoding']).",".quote_smart($_REQUEST['mailformat']).")") or die('error');
		if(mysql_affected_rows()) {
			echo"<script> alert('NEW EMAIL CAMPAIGN ADDED SUCCESFULLY!'); </script>";
		} else {
			echo"<script> alert('COULD NOT ADD NEW EMAIL CAMPAIGN'); </script>";
		}
	}
}
...
if($action == 'addCreditsNow') {
	...
	if($error != '') {
		...
		if($valid != TRUE) {
			...
			$query = mysql_query("SELECT fsize, `$idName` AS campaignID FROM `$table` WHERE `".$adTypes[$adType]['idType']."` = ".quote_smart($_REQUEST['id'])." AND username = ".quote_smart($_SESSION['login'])."") or die(mysql_error());
			...
			if($count > 0) {
				...
				//$creditID = getValue("SELECT creditID FROM `".$table."` WHERE `".$adTypes[$adType]['idType']."` = ".quote_smart($_REQUEST['id'])."");
				...
				if($adTypes[$adType]['hasTypes'] == TRUE) {
					...
					mysql_query("UPDATE `$table` SET `creditID` = ".quote_smart($_REQUEST['creditID'])." WHERE `username` = ".quote_smart($_SESSION['login'])." AND `".$adTypes[$adType]['idType']."` = ".quote_smart($_REQUEST['id'])."");
	...
	if($_REQUEST['id'] == '') {
		...
		if(is_numeric($_REQUEST['creditsToRetract']) && $activeCredits >= $_REQUEST['creditsToRetract']) {
			if($valid != TRUE) {
				...
				if($adType == 'links' || $adType == 'ptrad') { $idName = 'fn'; } else if($adType == 'survey') { $idName = 'id'; } else { $idName = 'fnum'; }
				$query = mysql_query("SELECT fsize, `$idName` AS campaignID, creditID FROM `$table` WHERE `$idName` = ".quote_smart($_REQUEST['id'])." AND username = ".quote_smart($_SESSION['login'])."") or die(mysql_error());
				$count = mysql_num_rows($query);
				if($count > 0) {
					$arr = mysql_fetch_array($query);
					...
					if($adTypes[$adType]['hasTypes'] == TRUE) {
						...
						mysql_query("UPDATE `$table` SET `creditID` = ".quote_smart($_REQUEST['creditID'])." WHERE `username` = ".quote_smart($_SESSION['login'])." AND `$idName` = ".quote_smart($_REQUEST['id'])."");
					...
					$creditID = getValue("SELECT creditID FROM `".$table."` WHERE `".$idName."`=".quote_smart($_REQUEST['id'])."");
					...
					if($_REQUEST['creditsToRetract'] > 0) {
						if($_REQUEST['creditsToRetract'] <= $activeCredits) {
							//echo "Setting the current ".$fsize." to ".($fsize+$_REQUEST['creditsToAdd']).".<BR>Added ".$_REQUEST['creditsToAdd']."...<BR>";
							$credit = creditAccount($_SESSION['login'], $adType, $_REQUEST['creditsToRetract']*$multiplier,$_REQUEST['id']);
							if($credit) { 
								mysql_query("UPDATE `$table` SET fsize = ".quote_smart(($fsize-$_REQUEST['creditsToRetract']))." WHERE username = ".quote_smart($_SESSION['login'])." AND $idName = ".quote_smart($_REQUEST['id'])."");
...
if($action == 'remove' && $adType != '' && $id != '' && is_numeric($id)) {
	if($adType == 'banner') {
		$array = getArray("SELECT fclicks, fshows, fsize FROM banners WHERE fnum = ".quote_smart($id)." AND username = ".quote_smart($_SESSION['login'])."");
		$credits = $array['fsize'] - $array['fshows'];
		if($credits > 0) refundCredits($_SESSION['login'], $adType, $credits);
		mysql_query("DELETE FROM banners WHERE fnum=".quote_smart($id)." AND username = ".quote_smart($_SESSION['login'])."");
		echo"<script> alert('BANNER REMOVED! Refunded ".$credits." credits.'); </script>";
	} else if($adType == 'fbanner') {
		$array = getArray("SELECT fclicks, fshows, fsize FROM fbanners WHERE fnum = ".quote_smart($id)." AND username = ".quote_smart($_SESSION['login'])."");
		$credits = $array['fsize'] - $array['fshows'];
		if($credits > 0) refundCredits($_SESSION['login'], $adType, $credits);
		mysql_query("DELETE FROM fbanners WHERE fnum=".quote_smart($id)." AND username = ".quote_smart($_SESSION['login'])."");
		echo"<script> alert('FEATURED BANNER REMOVED! Refunded ".$credits." credits.');</script>";
	} else if($adType == 'fad') {
		$array = getArray("SELECT fclicks, fshows, fsize FROM featuredads WHERE fnum = ".quote_smart($id)." AND username = ".quote_smart($_SESSION['login'])."");
		$credits = $array['fsize'] - $array['fshows'];
		if($credits > 0) refundCredits($_SESSION['login'], $adType, $credits);
		mysql_query("DELETE FROM featuredads WHERE fnum=".quote_smart($id)." AND username = ".quote_smart($_SESSION['login'])."");
		echo"<script> alert('FEATURED AD REMOVED! Refunded ".$credits." credits.');</script>";
	} else if($adType == 'flinks') {
		$array = getArray("SELECT fclicks, fshows, fsize FROM featuredlinks WHERE fnum = ".quote_smart($id)." AND username = ".quote_smart($_SESSION['login'])."");
		$credits = $array['fsize'] - $array['fshows'];
		if($credits > 0) refundCredits($_SESSION['login'], $adType, $credits);
		mysql_query("DELETE FROM featuredlinks WHERE fnum=".quote_smart($id)." AND username = ".quote_smart($_SESSION['login'])."");
		echo"<script> alert('FEATURED LINK REMOVED! Refunded ".$credits." credits.');</script>";
	} else if($adType == 'links') {
		$array = getArray("SELECT fvisits, fsize FROM tasks WHERE fn = ".quote_smart($id)." AND username = ".quote_smart($_SESSION['login'])."");
		$credits = $array['fsize'] - $array['fvisits'];
		if($credits > 0) refundCredits($_SESSION['login'], $adType, $credits);
		mysql_query("DELETE FROM tasks WHERE fn=".quote_smart($id)." AND username = ".quote_smart($_SESSION['login'])."");
		echo"<script> alert('PTC LINK REMOVED! Refunded ".$credits." credits.');</script>";
	} else if($adType == 'email') {
		$array = getArray("SELECT freads, fsize FROM `reads` WHERE fnum = ".quote_smart($id)." AND username = ".quote_smart($_SESSION['login'])."");
		$credits = $array['fsize'] - $array['freads'];
		if($credits > 0) refundCredits($_SESSION['login'], $adType, $credits);
		mysql_query("DELETE FROM `reads` WHERE fnum=".quote_smart($id)." AND username = ".quote_smart($_SESSION['login'])."");
		echo"<script> alert('PAID EMAIL AD REMOVED! Refunded ".$credits." credits.');</script>";
	} else if($adType == 'signup') {
		$array = getArray("SELECT fsignups, fsize FROM signups WHERE fnum = ".quote_smart($id)." AND username = ".quote_smart($_SESSION['login'])."");
		$credits = $array['fsize'] - $array['fsignups'];
		if($credits > 0) refundCredits($_SESSION['login'], $adType, $credits);
		mysql_query("DELETE FROM signups WHERE fnum=".quote_smart($id)." AND username = ".quote_smart($_SESSION['login'])."");
		echo"<script> alert('PAID SIGNUP AD REMOVED! Refunded ".$credits." credits.');</script>";
	} else if($adType == 'survey') {
		$array = getArray("SELECT fviews, fsize FROM surveys WHERE id = ".quote_smart($id)." AND username = ".quote_smart($_SESSION['login'])."");
		$credits = $array['fsize'] - $array['fviews'];
		if($credits > 0) refundCredits($_SESSION['login'], $adType, $credits);
		mysql_query("DELETE FROM surveys WHERE id=".quote_smart($id)." AND username = ".quote_smart($_SESSION['login'])."");
		echo"<script> alert('PAID SURVEY REMOVED! Refunded ".$credits." credits.');</script>";
	} else {
